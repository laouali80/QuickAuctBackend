...
class AuctionQuerySet(models.QuerySet):
    ...
    def active(self):
        ...
        now = timezone.now()
        return self.filter(
            status=Auction.Status.ONGOING, start_time__lte=now, end_time__gt=now
        )
    ...
    def user_latest_bids(self, user):
        ...
        user_bids = Bid.objects.filter(auction=OuterRef("pk"), bidder=user).order_by(
            "-placed_at"
        )
...
class Auction(models.Model):
    ...
    current_price = models.DecimalField(
        max_digits=12, decimal_places=2, validators=[MinValueValidator(0.01)], null=True
    )
    ...
    category = models.ForeignKey(
        Category, on_delete=models.SET_NULL, null=True, related_name="auctions"
    )
...
class AuctionTransaction(models.Model):
    auction = models.OneToOneField(
        Auction, on_delete=models.CASCADE, related_name="transaction"
    )
    buyer = models.ForeignKey(
        "User", on_delete=models.SET_NULL, null=True, related_name="purchases"
    )
    seller = models.ForeignKey(
...
class AuctionReport(models.Model):
    ...
    auction_seller = models.ForeignKey(
